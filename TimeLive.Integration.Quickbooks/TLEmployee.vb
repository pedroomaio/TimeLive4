...
Public Class TLEmployee
    ...
    Public Sub GetAndAddEmployeeInTimeLive()
        ...
        Try
            ...
            Dim vendormsgSetRq As IMsgSetRequest = sessManager.CreateMsgSetRequest("US", 2, 0)
            ...
            vendormsgSetRs = sessManager.DoRequests(vendormsgSetRq)
            ...
            If (emprespList Is Nothing) And (vendorrespList Is Nothing) Then
                Exit Sub
            End If
            ...
            If (vendorresp.StatusCode = 0) Then
                ...
                For i As Integer = 0 To vendorRetList.Count - 1
                    ...
                    With vendorRet
                        Try
                            EmailAddress = GetEmailAddress(.Email, True)
                            objEmployeeServices.InsertEmployee(EmailAddress, EmailAddress, GetValue(.FirstName, "FirstName", True), _
                            GetValue(.LastName, "LastName", True), EmailAddress, "", nDepartmentId, nRoleId, nLocationId, _
                            233, nBillingTypeId, Now.Date, -1, 0, 6, 0, 0, nEmployeeTypeId, nEmployeeStatusId, _
                            "", Now.Date, Now.Date, nWorkingDayTypeId, System.Guid.Empty, 0, System.Guid.Empty, False, "", "", "", "", "", "", "", "", "", "Mr.", True)
                            'TextBox1.Text = TextBox1.Text & vbCrLf & "[" & vendorRet.Name.GetValue & "] transferred successfully."
                            RichTextBox1.Text = RichTextBox1.Text & vbCrLf & "[" & vendorRet.Name.GetValue & "] transferred successfully."
                        Catch ex As Exception
                            'TextBox1.Text = TextBox1.Text & vbCrLf & "[" & vendorRet.Name.GetValue & "] transferred failed."
                            RichTextBox1.Text = RichTextBox1.Text & vbCrLf & "[" & vendorRet.Name.GetValue & "] transferred failed."
                        End Try
                    End With
                Next
            End If
