...
(function ($) {
    ...
    Calendar.prototype = {
        ...
        _openContextMenu: function (elt) {
            ...
            for (var i in events) {
                if (!(date.getDay() == 6 || date.getDay() == 0)) {
                    ...
                    this._createContextMenuItems(contextMenu,events[i])
                }
                else {
                    if (events[i].scheduleWeekends) {
                        this._createContextMenuItems(contextMenu, events[i])
                    }
                }
            }
        ...
        _createContextMenuItems: function (contextMenu, evt) {
            var eventItem = $(document.createElement('div'));
            eventItem.addClass('item');
            eventItem.css('border-left', '4px solid ' + evt.color);

            var eventItemContent = $(document.createElement('div'));
            eventItemContent.addClass('content');
            eventItemContent.text(evt.name);
            ...
            this._renderContextMenuItems(eventItem, this.options.contextMenuItems, evt);
