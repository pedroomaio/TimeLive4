CREATE VIEW [dbo].[vxp_DetailMonthAllocationWithApprover]
AS
...
SELECT     dbo.AccountEmployee.AccountEmployeeId AS _EMPLOYEE_ID, dbo.AccountEmployee.EmployeeCode AS _EMPLOYEE_CODE, REPLACE(CONVERT(varchar(10), 
                      ...
                      dbo.AccountTaskType.TaskType AS _TASK_CODE, AccountEmployee_1.EmployeeCode AS _PROJECT_MANAGER, Approver.EMPLOYEECODE as _Approver
FROM         dbo.AccountEmployeeTimeEntry INNER JOIN
					  ...
					  LEFT JOIN vueAccountEmployeeTimeEntryApprovalPendingApprover vue ON VUE.AccountEmployeeTimeEntryID =AccountEmployeeTimeEntry.AccountEmployeeTimeEntryID
					  LEFT JOIN  dbo.AccountEmployee AS Approver ON Approver.AccountEmployeeId = vue.approverEmployeeID
...
GROUP BY dbo.AccountEmployee.AccountEmployeeId, dbo.AccountEmployee.EmployeeCode, DATEPART(month, dbo.AccountEmployeeTimeEntry.TimeEntryDate), 
                      ...
                      dbo.AccountEmployeeTimeEntry.Submitted, dbo.AccountTaskType.TaskType, AccountEmployee_1.EmployeeCode,Approver.EMPLOYEECODE