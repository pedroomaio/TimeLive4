...
CREATE PROCEDURE [dbo].[sp_xpTimeSheetProjectDataSet] 
...
set @query = @query + ' AND ACCOUNTEMPLOYEEID in ( select vxp.ACCOUNTEMPLOYEEID from VXP_DETAILTIMESHEET vxp where  vxp.ACCOUNTPROJECTID = ' + convert(varchar,@pProject) +'AND vxp.TIMEENTRYDATE >= convert(datetime, '''+ convert(varchar,@pStartDate,103) + ''' ,103) AND  vxp.TIMEENTRYDATE <= convert(datetime, '''+ convert(varchar,@pEndDate,103) + ''' ,103)) '
if(@pProject != 0)
 begin
